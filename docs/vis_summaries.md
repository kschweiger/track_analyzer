...
Additionally the some aggregation metrics can be split per segment in a track by passing `kind="segment-zone-summary"`.  Internally this will call the  [`plot_segment_zones`][geo_track_analyzer.visualize.plot_segment_zones] function. See the api documentation for all options that can be passed.

--8<-- "docs/snippets/examples/visualization/zone_segment_summary_hr_time.html"

???+ warning "Errors will be raised"

    An `VisualizationSetupError` is passed if `aggregate` and `metric` keyword-argument pairs are **not passed** to the `plot` method for kinds `"zone-summary"` and `"segment-zone-summary"`
